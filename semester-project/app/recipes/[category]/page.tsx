...
import { faFilter, faComment } from '@fortawesome/free-solid-svg-icons';
...
export default function RecipesCategories({ params }: RecipesCategoriesParams) {
  ...
  return (
    <main className="flex flex-col items-center min-h-screen max-w-8xl mx-auto p-10">
      <h1 className="text-5xl md:text-7xl font-bold p-5 md:p-10 capitalize text-custom-main-color font-arial-rounded">{params.category} Recipes</h1>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between w-full mb-5">
        <div className="flex flex-col justify-between md:flex-row md:items-center md:space-x-4 w-full">
          <SearchBox onSearch={handleSearch} />
          <div className="text-font-color text-xl mt-4 md:mt-0">
        ...
        <div className="flex justify-end mt-16">
          <button onClick={handleShowFilterBoxClick} className="text-xl font-bold text-custom-main-color flex items-center">
            <FontAwesomeIcon icon={faFilter} className="mr-2" />
            Filters
          </button>
        </div>
      </div>
      {showFilterBox && <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 blur-5 z-50 overflow-hidden"><FilterBox
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        {filteredEntries
          ...
          .filter(entry => entry.fields.name.toLowerCase().includes(searchQuery.toLowerCase()))
          .map(entry => (
            <div key={entry.sys.id} className="relative w-full border-2 mt-10 md:mt-16 bg-white p-5 shadow-xl text-center rounded-3xl">
